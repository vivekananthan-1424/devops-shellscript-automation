...
if cp -r "$TMP_DIR/$TEMPLATE_FOLDER/"* "$WEB_ROOT"; then
    log "Admin_Dashboard deployed successfully to $WEB_ROOT"
    send_slack "Admin_Dashboard deployed successfully to $WEB_ROOT destination."
    ...
    send_slack "Website deployment failed for $WEB_ROOT. Rolled back to previous version."
    send_email "Website Deployment Failure" "Failed to deploy the website to $WEB_ROOT. Please check the logs for details."
    exit 1
fi
